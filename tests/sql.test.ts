...
describe('quoting', () => {
  ...
  it('escapes literals safely', () => {
    ...
    expect(quoteLiteral('1499.00', 'postgres', 'numeric(10,2)')).toBe('1499.00')
    expect(quoteLiteral('12abc', 'postgres', 'integer')).toBe("'12abc'")
    expect(quoteLiteral('true', 'mysql', 'boolean')).toBe('1')
    expect(quoteLiteral('42', 'postgres', 'text')).toBe("'42'")
  })
})
